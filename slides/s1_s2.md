...
- **Mejora concreta (sin implementar hoy):**
  - Ejecutar scripts múltiples veces y agregar análisis de variabilidad en tiempos de respuesta. Esta práctica permite identificar comportamientos intermitentes (flaky tests) y asegurar que los resultados sean consistentes, ya que una sola ejecución no ofrece la base estadística necesaria para confirmar la estabilidad del sistema bajo condiciones repetitivas.
  - Documentar explícitamente configuración del SUT (versión Node.js, memoria Docker, etc.) para reproducibilidad. Se busca garantizar la reproducibilidad técnica del escenario en cualquier entorno, dado que la calidad es dependiente del contexto físico y lógico; sin estos parámetros, la evidencia pierde su valor comparativo y científico.
  - Definir umbrales de latencia (e.g., p95 < Xms) basados en requisitos de negocio y justificarlos. Esta métrica permite validar la experiencia de usuario real más allá de la conectividad técnica, utilizando el estándar p95 para asegurar que la gran mayoría de las peticiones cumplen con un tiempo de respuesta aceptable, separando el rendimiento estable de los casos atípicos.
